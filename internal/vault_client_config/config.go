...
// NOTE: This was copied from vault-secrets-webhook/pkg/webhook/config.go
package vault_client_config
...
func parseVaultConfig(obj metav1.Object, ar *model.AdmissionReview) VaultConfig {
	...
	if val, ok := annotations["vault.security.banzaicloud.io/vault-role"]; ok {
		...
		if val := viper.GetString("vault_role"); val != "" {
			...
			vaultConfig.Role = serviceAccountName
		} else {
	...
	if val, ok := annotations["alpha.vault.security.banzaicloud.io/reload-on-secret-change"]; ok {
		vaultConfig.SecretReload, _ = strconv.ParseBool(val)
	} else {
		vaultConfig.SecretReload = false
	}
...
// NOTE: This was copied from vault-secrets-webhook/pkg/webhook/webhook.go
